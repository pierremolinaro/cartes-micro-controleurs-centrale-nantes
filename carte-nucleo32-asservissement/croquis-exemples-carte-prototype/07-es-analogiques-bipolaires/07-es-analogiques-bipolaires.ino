...
//--- Deuxième action : lire l'encodeur pour fixer les sorties analogiques
  ...
  if (gValeurEncodeur != encodeur) {
    ...
    actionSortieAnalogiqueBipolaire (SORTIE_ANALOGIQUE::S0, commande) ;
    actionSortieAnalogiqueBipolaire (SORTIE_ANALOGIQUE::S1, commande) ;
    lcd.setCursor (10, 1) ;
    lcd.print ("   ") ;
    lcd.setCursor (10, 1) ;
    lcd.print (commande) ;
...
//--- Troisième action : lire les entrées analogiques
  if (gInstantLectureAnalogique <= millis ()) {
    ...
    const uint16_t entree0 = lireEntreeAnalogiqueBipolaire (ENTREE_ANALOGIQUE::E0) ;
    ...
    const uint16_t entree1 = lireEntreeAnalogiqueBipolaire (ENTREE_ANALOGIQUE::E1) ;
