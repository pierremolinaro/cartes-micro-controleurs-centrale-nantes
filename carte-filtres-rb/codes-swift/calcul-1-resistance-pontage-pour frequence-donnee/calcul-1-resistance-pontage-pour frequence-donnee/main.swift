//
//  main.swift
//  calcul-1-resistance-pontage-pour frequence-donnee
//
...
let FRÉQUENCE_HAUTE : Int = 8_000

let FRÉQUENCE_CIBLE : Double = 944.0
...
func meilleursRésultats (_ inFréquence : Double, _ nombreRésulats : Int) -> [Résultat] {
  ...
  for i in 0 ..< RÉSISTANCES.count {
    ...
    let fréquenceCoupures = [
      (1.0 / (2.0 * .pi * (par (R1H, R1x) * R2H * C1 * C2).squareRoot ()), false, false),
      (1.0 / (2.0 * .pi * (par (R1H, R1L, R1x) * R2H * C1 * C2).squareRoot ()), true, false),
      (1.0 / (2.0 * .pi * (par (R1H, R1x) * par (R2H, R2L) * C1 * C2).squareRoot ()), false, true),
      (1.0 / (2.0 * .pi * (par (R1H, R1L, R1x) * par (R2H, R2L) * C1 * C2).squareRoot ()), true, true)
    ]
    for (f, r1lOn, r2lOn) in fréquenceCoupures {
      let écart = 100.0 * abs (f - inFréquence) / inFréquence
      var résultat = Résultat (
        R1x: R1x, R1Lon: r1lOn, R2x: 0.0, R2Lon: r2lOn,
  ...
  for i in 0 ..< RÉSISTANCES.count {
    let R2x = RÉSISTANCES [i]
    let fréquenceCoupures = [
      (1.0 / (2.0 * .pi * (R1H * par (R2H, R2x) * C1 * C2).squareRoot ()), false, false),
      (1.0 / (2.0 * .pi * (par (R1H, R1L) * par (R2H, R2x) * C1 * C2).squareRoot ()), true, false),
      (1.0 / (2.0 * .pi * (R1H * par (R2H, R2L, R2x) * C1 * C2).squareRoot ()), false, true),
      (1.0 / (2.0 * .pi * (par (R1H, R1L) * par (R2H, R2L, R2x) * C1 * C2).squareRoot ()), true, true),
    ]
    for (f, r1lOn, r2lOn) in fréquenceCoupures {
      let écart = 100.0 * abs (f - inFréquence) / inFréquence
      var résultat = Résultat (
        R1x: 0.0, R1Lon: r1lOn, R2x: R2x, R2Lon: r2lOn,
        fCible: inFréquence, fc: f,
        écart: écart
      )
      for i in 0 ..< tableauRésultats.count {
...
for résultat in meilleursRésultats (FRÉQUENCE_CIBLE, 5) {
