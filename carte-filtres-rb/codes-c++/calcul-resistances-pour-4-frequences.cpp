...
//--- Fréquences cible
  const double factor = pow (FREQUENCE_CIBLE_HAUTE / FREQUENCE_CIBLE_BASSE, 1.0 / 3.0) ;
  const double FREQUENCES [4] = {
    FREQUENCE_CIBLE_BASSE,
    FREQUENCE_CIBLE_BASSE * factor,
    FREQUENCE_CIBLE_BASSE * factor * factor,
  ...
  for (int i = 1 ; i < RESISTANCES.size () ; i++) {
    ...
    for (int j = 0 ; j < i ; j++) {
      ...
      for (int k = 1 ; k <= i ; k++) {
        ...
        //--- Ordonner les 4 fréquences dans l'ordre croissant
          bool loop = true ;
          while (loop) {
            loop = false ;
            for (int w = 1 ; w < 3 ; w++) {
              if (candidates [w-1] > candidates [w]) {
                loop = true ;
                const double temp = candidates [w-1] ;
                candidates [w-1] = candidates [w] ;
                candidates [w] = temp ;
              }
            }
          }
...
//--- Affichage des résultats
  ...
  for (int i = 0 ; i < tableauResultats.size () ; i++) {
    printf (
      "| %0.6f | %4.1f | %4.1f | %5.1f | %5.1f | %6.0f | %6.0f | %6.0f | %6.0f |\n",
